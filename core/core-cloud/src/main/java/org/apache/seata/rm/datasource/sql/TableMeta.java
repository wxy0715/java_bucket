     ...
     public Map<String, ColumnMeta> getPrimaryKeyMap() {
         ...
         this.allIndexes.forEach((key, index) -> {
             if (index.getIndextype().value() == IndexType.PRIMARY.value()
                     || (index.getIndextype().value() == IndexType.UNIQUE.value() && ("ID".equalsIgnoreCase(index.getValues().get(0).getColumnName()) || "ID_".equalsIgnoreCase(index.getValues().get(0).getColumnName())))) {
