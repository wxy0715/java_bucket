package com.cjree.core.file.aop;
...
public class LogFileStorageAspect implements FileStorageAspect {
     ...
     */
    @Override
    public GeneratePresignedUrlResult generatePresignedUrlAround(GeneratePresignedUrlAspectChain chain, GeneratePresignedUrlPretreatment pre, FileStorage fileStorage) {
        GeneratePresignedUrlResult next = chain.next(pre, fileStorage);
        log.info("对文件生成可以签名访问的 URL after -> {}", next.getUrl());
        return next;
    }
